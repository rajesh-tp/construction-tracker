...
import { deleteConstruction } from "@/lib/actions";
import { toast } from "sonner";
import { useTransition, useState, useEffect, useCallback } from "react";
import { useRouter } from "next/navigation";

export function DeleteConstructionButton({ id, name }: { id: number; name: string }) {
  ...
  function handleConfirm() {
    startTransition(async () => {
      const result = await deleteConstruction(id);
        ...
        >
          ...
          >
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-text-heading">Delete Construction</h3>
                <p className="text-sm text-text-muted">This action cannot be undone.</p>
              </div>
            </div>

            <p className="mb-6 text-sm text-text-secondary">
              Are you sure you want to delete <strong>&ldquo;{name}&rdquo;</strong>? All associated contractors, accounts, and user assignments will be removed. Constructions with transactions cannot be deleted.
            </p>
